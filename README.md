...
Still a WIP right now but working pretty well. The script should be placed in the same folder as the MindView CSV and that CSV should be renamed to 'sleepData.csv'. The script may likely need to be modified so you reference the right columns if your setup for Mind View csv exports are different - which they quite likely will be.
